...
const getDateTime = (timeSring, shortMinuteMultiply10 = false/*true*/) => {
...
const checkMeetingTime = (workTimeBeginString, workTimeEndString, meetTimeBeginString, meetDurationMinutes) => {
  ...
  const workDateTimeBegin = getDateTime(workTimeBeginString);
  const workDateTimeEnd = getDateTime(workTimeEndString);

  if (workDateTimeBegin > workDateTimeEnd) {
    return false;
  }

  const meetDateTimeBegin = getDateTime(meetTimeBeginString);
  const meetDateTimeEnd = new Date(meetDateTimeBegin);
  meetDateTimeEnd.setMinutes(meetDateTimeEnd.getMinutes() + meetDurationMinutes);
  ...
  return ((meetDateTimeBegin >= workDateTimeBegin) && (meetDateTimeEnd <= workDateTimeEnd));
};
